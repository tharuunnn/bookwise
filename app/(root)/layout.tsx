...
const Layout = async ({ children }: { children: ReactNode }) => {
  ...
  after(async () => {
    ...
    if (user[0].lastActivityDate === new Date().toISOString().slice(0, 10)) //checks if the last updated day is today to prevent unnecessary additions.
      return;
    ...
    await db
      ...
      .where(eq(users.id, session?.user?.id));
  });//toiso bla bla converts time to utc and cuts off time so only the date remains
